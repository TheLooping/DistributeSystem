...
namespace distributedsystem
{
    namespace mapreduce
    {
        ...
        {
            // TODO : 改参数名 rs_loc 为 result_location
        ...
        void MasterHandler::CommitMapTask(const TaskResult &result)
        {
            ...
            {
                LOG(INFO) << fmt::format("Commit map task {} result: {}", result.id, fmt::join(result.rs_loc, ", "));
                vector<string> tmp_result(result.rs_loc.size());
                for (int i = 0; i < result.rs_loc.size(); ++i)
                {
                    string new_name = fmt::format("mr.mid{}.rid{}", result.id, i);
                    std::rename(result.rs_loc[i].c_str(), new_name.c_str());
        ...
        {
            if (result.rs_loc.size() != 1)
            {
                LOG(ERROR) << "Reduce task result location size is not 1: " << result.rs_loc.size();
                return;
            }
            string new_name = fmt::format("mr.out{}", result.id);
            std::rename(result.rs_loc[0].c_str(), new_name.c_str());
        ...
        vid MasterHandler::SetExitServer(std::function<void(void)> exit_server)
        {
